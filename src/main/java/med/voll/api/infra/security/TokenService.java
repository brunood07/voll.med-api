...
public class TokenService {
    ...
    public String generateToken(User user) {
        try {
            ...
            return JWT.create().withIssuer("API Voll.med").withSubject(user.getLogin()).withExpiresAt(expirationDate()).sign(algorithm);
        } catch (JWTCreationException exception) {
            throw new RuntimeException("Error while generating JWT token", exception);
        }
    }

    public String decryptToken(String tokenJWT) {
        try {
            var algorithm = Algorithm.HMAC256(secret);
